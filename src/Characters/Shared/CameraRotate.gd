...
func reset_camera(delta):
	...
	if (Input.is_action_just_pressed("reset_camera") && enable_cardinal == true):
		...
		TWEEN.interpolate_property(CAMERABOOM, "rotation_degrees", current_rot, 
			Vector3(current_rot.x,nearest_cardinal,current_rot.z), 0.2, Tween.TRANS_EXPO, Tween.EASE_IN_OUT)
		TWEEN.start();
...
#		CAMERABOOM.rotation.y = lerp_angle(current_rot, 0, delta*10)
	if (Input.is_action_just_pressed("reset_camera") && enable_cardinal == false):
		reset = true;
		TWEEN.interpolate_property(CAMERABOOM, "rotation_degrees", current_rot, 
			Vector3(current_rot.x,0,current_rot.z), 0.2, Tween.TRANS_EXPO, Tween.EASE_IN_OUT)
		TWEEN.start();
		yield(TWEEN, "tween_completed")
		reset = false;
	pass
